...
export function BookmarkCard({ bookmark, onReview }: BookmarkCardProps) {
  ...
  return (
    <div
      className={cn(
        "group relative rounded-2xl border overflow-hidden transition-all duration-300 backdrop-blur-sm sb-glass sb-grid",
        isDeleting
          ? "border-red-400/15 opacity-40 scale-[0.98] pointer-events-none bg-white/[0.005]"
          : "border-white/[0.06] bg-white/[0.01] hover:border-white/[0.12] hover:-translate-y-1 hover:shadow-lg hover:shadow-[var(--sb-accent)]/5",
      )}
    >
      ...
      <div className="p-4 sm:p-5 space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {bookmark.question.topic && (
              <span className="px-2.5 py-1 rounded-lg bg-white/[0.03] text-[9px] font-bold uppercase tracking-wider text-white/30 border border-white/[0.04] max-w-[120px] truncate">
                {bookmark.question.topic}
              </span>
            )}
          </div>
          ...
          {expiryInfo && (
            ...
            >
              ...
              <span>{expiryInfo.daysLeft}d remaining</span>
            </div>
          )}
        </div>
        ...
        <div className="space-y-2">
          <div className="font-serif text-sm leading-relaxed text-white/85">
            <MathMarkdown content={bookmark.question.questionText} />
          </div>

          {/* Image indicator */}
          {bookmark.question.hasImage && (
            <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded-lg bg-[var(--sb-accent)]/[0.05] border border-[var(--sb-accent)]/10">
              <ImageIcon className="h-3 w-3 text-[var(--sb-accent)]/60" />
              <span className="text-[9px] text-[var(--sb-accent-text)]/70 font-semibold uppercase tracking-wider">
                Media attached
              </span>
            </div>
          )}
          ...
          {isEditingNotes ? (
            ...
            >
              <div className="relative">
                <textarea
                  ...
                  rows={3}
                  maxLength={2000}
                  placeholder="Annotate this card with custom rules or reminders..."
                ...
                />
                <span className="text-[8px] text-white/20 mt-1 block text-right font-semibold">
              ...
              <div className="flex items-center gap-2 justify-end">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-wider text-white/30 hover:text-white/60 hover:bg-white/[0.03] transition-all border border-white/[0.04]"
                >
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <Pencil className="h-3.5 w-3.5 shrink-0 text-white/20 group-hover/notes:text-[var(--sb-accent)]/70 transition-colors mt-0.5" />
              </div>
            </motion.button>
          ) : null}
        </AnimatePresence>
        ...
        <div className="flex items-center gap-2 pt-3 border-t border-white/[0.04]">
          <button
            ...
            className="flex-1 px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider text-white bg-white/[0.03] border border-white/[0.05] hover:bg-white/[0.06] hover:border-white/[0.10] active:scale-[0.98] transition-all"
          >
            Review Solution
          </button>
          ...
          {!bookmark.notes && !isEditingNotes && (
            <button
              onClick={() => setIsEditingNotes(true)}
              className="flex items-center justify-center h-9 w-9 rounded-xl text-white/40 bg-white/[0.02] border border-white/[0.04] hover:bg-white/[0.05] hover:text-white/60 active:scale-[0.98] transition-all"
              title="Add notes"
            >
          ...
          <button
            ...
            className={cn(
              ...
              deleteBookmark.isPending || isDeleting
                ? "text-red-400/30 bg-red-400/[0.02] border-red-400/10 cursor-not-allowed opacity-50"
                : "text-red-400/60 bg-red-400/[0.04] hover:bg-red-400/[0.08] hover:text-red-400 border-red-400/10 hover:border-red-400/25 active:scale-[0.98]",
            )}
