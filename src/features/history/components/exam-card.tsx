      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {exam.subjects.slice(0, 2).map((s) => (
             <span key={s} className="text-[10px] uppercase font-bold text-white/30">{s.slice(0,3)}</span>
          ))}
          {exam.subjects.length > 2 && (
             <span className="text-[10px] uppercase font-bold text-white/30">+{exam.subjects.length - 2}</span>
          )}
        </div>
      ...
      <div className="flex items-end justify-between border-b border-white/[0.06] pb-5 mb-5">
         <div>
            ...
            <p className="text-xs text-white/40">
               Session #{exam.sessionNumber} {exam.isRetake ? `(Attempt ${exam.attemptNumber})` : ""}
            </p>
         </div>
         {isCompleted ? (
            <div className="text-right">
               <div className={cn("sb-mono text-2xl md:text-3xl font-bold tracking-tighter leading-none mb-1", colorClass)}>
                  {exam.percentage}%
               </div>
         ...
         <div className="flex items-center gap-4 text-xs font-medium text-white/40">
            ...
            {exam.timeTakenSeconds !== null && (
               <span className="flex items-center gap-1.5" title="Time Taken">
                  ...
                  {Math.floor(exam.timeTakenSeconds / 60)}m {exam.timeTakenSeconds % 60}s
               </span>
            )}
         </div>
